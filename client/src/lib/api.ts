...
export const getDeletedTaskLogsQueryFn = async ({
  workspaceId,
  pageSize = 20,
  pageNumber = 1,
}: {
  ...
  pageSize?: number;
  pageNumber?: number;
}): Promise<import("../types/api.type").DeletedTaskLogsResponseType> => {
  const response = await API.get(
    `/task/workspace/${workspaceId}/deleted-logs?pageSize=${pageSize}&pageNumber=${pageNumber}`
  );
  return response.data;
};

export const clearDeletedTaskLogsMutationFn = async ({
  workspaceId,
}: {
  workspaceId: string;
}): Promise<{ message: string; deletedCount: number }> => {
  const response = await API.delete(
    `/task/workspace/${workspaceId}/deleted-logs`
  );
  return response.data;
};
