...
export default function CreateTaskForm(props: {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      assignedTo: "AUTO", // Set default to trigger smart routing
    },
  });
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    const payload = {
      ...
      data: {
        ...values,
        // Send null if AUTO is selected, otherwise send the specific user ID
        assignedTo: values.assignedTo === "AUTO" ? null : values.assignedTo,
    ...
    mutate(payload, {
      onSuccess: () => {
        // Invalidate analytics so the bottleneck prediction updates
        queryClient.invalidateQueries({
          queryKey: ["project-analytics", projectId],
        });

        // Invalidate task list to show the newly assigned task
        queryClient.invalidateQueries({
                      ...
                      <SelectContent>
                        <div
                          className="w-full max-h-[200px]
                           ...
                           "
                        >
                          {/* Inject the Auto-Assign option here */}
                          <SelectItem
                            value="AUTO"
                            className="cursor-pointer font-medium text-primary"
                          >
                            🤖 Auto-Assign (Fastest Developer)
                          </SelectItem>
