...
const TaskTable = () => {
  ...
  const { mutateAsync: deleteSingleTask } = useMutation({
    mutationFn: deleteTaskMutationFn,
  });

  const [isDeletingSelected, setIsDeletingSelected] = useState(false);

  const handleBulkDelete = async (selectedRows: TaskType[]) => {
    if (!canDeleteTask || selectedRows.length === 0) return;
    setIsDeletingSelected(true);
    try {
      await Promise.all(
        selectedRows.map((task) =>
          deleteSingleTask({ workspaceId, taskId: task._id as string })
        )
      );
      ...
      toast({
        title: "Tasks deleted",
        description: `${selectedRows.length} task${selectedRows.length > 1 ? "s" : ""} deleted successfully.`,
        variant: "success",
      });
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : "Failed to delete some tasks.";
      toast({
        title: "Error",
        description: message,
        variant: "destructive",
  ...
  return (
    <div className="w-full relative">
      <DataTable
        ...
        onDeleteSelected={canDeleteTask ? handleBulkDelete : undefined}
