  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    mutate(inviteCode, {
      ...
      onError: (error: { errorCode?: string; message: string }) => { 
        // Handle specific custom backend error codes
        if (error.errorCode === "VALIDATION_ERROR") {
          toast({
            title: "Already a member",
            description: "You are already a part of this workspace.",
          });
          ...
          navigate(`/`); // Redirect to the main dashboards
        } else {
          ...
          <Card>
            <CardHeader className="text-center">
              <CardTitle className="text-xl">
                Hey there! You're invited to join a SyncClick Workspace!
              </CardTitle>
              <CardDescription>
                Looks like you need to be logged into your SyncClick account to
                join this Workspace.
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="flex items-center justify-center my-3">
                      <form onSubmit={handleSubmit}>
                        <Button
                          type="submit"
                          disabled={isLoading}
                          className="!bg-primary !text-primary-foreground text-[23px] !h-auto"
                        >
                          {isLoading && (
